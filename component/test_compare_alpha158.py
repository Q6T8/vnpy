...
def create_test_data(n_days: int = 100, n_symbols: int = 3) -> tuple[pd.DataFrame, pl.DataFrame]:
...
def compare_feature(
    ...
    try:
        ...
        # Vnpy 版本计算
        try:
            from vnpy.alpha.dataset.utility import calculate_by_expression as vnpy_calculate
        ...
        if component_error and vnpy_error:
            ...
            result["error"] = f"Component: {component_error[:100]}; Vnpy: {vnpy_error[:100]}"
            return result
        ...
        if both_valid.sum() > 0:
            ...
            if np.any(large_diff_mask):
                ...
                result["large_diff_component"] = valid_component[large_diff_mask][:5].tolist()
...
def test_all_features():
    ...
    for i, (feature_name, expression) in enumerate(sorted_features, 1):
        ...
        if result["status"] != "match" or i % 10 == 0:
            print(f"{status_icon} [{i:3d}/{len(sorted_features)}] {feature_name:20s} - {status_text}", end="")
            if result["max_diff"] is not None:
                print(f" (最大差异: {result['max_diff']:.2e}, 平均差异: {result['mean_diff']:.2e})")
                if result.get("large_diff_indices"):
                    print(f"    偏差>0.01样本 idx={result['large_diff_indices']} comp={result['large_diff_component']} vnpy={result['large_diff_vnpy']}")
            elif result["error"]:
        ...
        for result in results:
            if result["status"] in ["diff", "error"]:
                ...
                if result["status"] == "error":
                    ...
                    print(f"  有效值数量: {result['match_count']}/{result['total_count']}")
    ...
    if close_count > 0:
        ...
        for result in results:
            if result["status"] == "close":
                print(f"{result['feature']:20s} - 最大差异: {result['max_diff']:.6e}, 平均差异: {result['mean_diff']:.6e}")
...
def test_sample_features():
    ...
    for feature_name in sample_features:
        ...
        if result["max_diff"] is not None:
            print(f" - 最大差异: {result['max_diff']:.6e}, 平均差异: {result['mean_diff']:.6e}")
        elif result["error"]:
